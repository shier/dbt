  ...
  if object_id ('"dbo"."BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbo"."BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbo"."BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp"','U') is not null
    begin
    drop table "dbo"."BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp"
    end


   EXEC('create view dbo.BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp_temp_view as
...
on t1.No_=t2.No_
  ...
  CREATE TABLE "dbo"."BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM dbo.BC_BJAuctionCompany_SalesInvoiceHeader_FT__dbt_tmp_temp_view)
