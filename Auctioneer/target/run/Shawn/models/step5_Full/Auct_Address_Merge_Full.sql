  ...
  if object_id ('"stg"."Auct_Address_Merge_Full__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "stg"."Auct_Address_Merge_Full__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"stg"."Auct_Address_Merge_Full__dbt_tmp"','U') is not null
    begin
    drop table "stg"."Auct_Address_Merge_Full__dbt_tmp"
    end


   EXEC('create view stg.Auct_Address_Merge_Full__dbt_tmp_temp_view as
    

With first_spotted_Address AS (
    SELECT 
        [AddressID],
        [Created]
    FROM (
...
FROM Address_merged
  ...
  CREATE TABLE "stg"."Auct_Address_Merge_Full__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM stg.Auct_Address_Merge_Full__dbt_tmp_temp_view)
