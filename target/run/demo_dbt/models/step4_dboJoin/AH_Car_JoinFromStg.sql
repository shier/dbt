  ...
  if object_id ('"dbo"."AH_Car_JoinFromStg__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbo"."AH_Car_JoinFromStg__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbo"."AH_Car_JoinFromStg__dbt_tmp"','U') is not null
    begin
    drop table "dbo"."AH_Car_JoinFromStg__dbt_tmp"
    end


   EXEC('create view dbo.AH_Car_JoinFromStg__dbt_tmp_temp_view as
...
JOIN [stg].[AH_Listingproperties_FinalView] AS H_LP ON H_LP.[CustomFieldID]=H_CF.[ID]
  ...
  CREATE TABLE "dbo"."AH_Car_JoinFromStg__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM dbo.AH_Car_JoinFromStg__dbt_tmp_temp_view)
