  ...
  if object_id ('"dbo"."CC_CompanyUser_JoinFromStg__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbo"."CC_CompanyUser_JoinFromStg__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbo"."CC_CompanyUser_JoinFromStg__dbt_tmp"','U') is not null
    begin
    drop table "dbo"."CC_CompanyUser_JoinFromStg__dbt_tmp"
    end


   EXEC('create view dbo.CC_CompanyUser_JoinFromStg__dbt_tmp_temp_view as
...
JOIN [stg].[CC_User_FinalView] AS C_U ON C_U.[CompanyID]=C_C.[ID]
  ...
  CREATE TABLE "dbo"."CC_CompanyUser_JoinFromStg__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM dbo.CC_CompanyUser_JoinFromStg__dbt_tmp_temp_view)
