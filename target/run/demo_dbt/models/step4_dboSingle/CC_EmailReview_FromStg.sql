  ...
  if object_id ('"dbo"."CC_EmailReview_FromStg__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbo"."CC_EmailReview_FromStg__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbo"."CC_EmailReview_FromStg__dbt_tmp"','U') is not null
    begin
    drop table "dbo"."CC_EmailReview_FromStg__dbt_tmp"
    end


   EXEC('create view dbo.CC_EmailReview_FromStg__dbt_tmp_temp_view as
...
From stg.[CC_EmailReview_FinalView]
  ...
  CREATE TABLE "dbo"."CC_EmailReview_FromStg__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM dbo.CC_EmailReview_FromStg__dbt_tmp_temp_view)
