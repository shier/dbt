  ...
  if object_id ('"stg"."Auct_AcceptionApplicationsBits_Incr"','U') is not null
   ...
   EXEC('create view stg.Auct_AcceptionApplicationsBits_Incr_temp_view as
    ...
    CONVERT(VARCHAR(32), HashBytes(''MD5'', 
        ...
        coalesce(cast(CONVERT(DATETIME2, ''2022-07-29 18:10:57.868877'') as varchar(max)), '''') 
    ), 2)
 as dbt_scd_id,
        CONVERT(DATETIME2, ''2022-07-29 18:10:57.868877'') as dbt_updated_at,
        CONVERT(DATETIME2, ''2022-07-29 18:10:57.868877'') as dbt_valid_from,
        nullif(CONVERT(DATETIME2, ''2022-07-29 18:10:57.868877''), CONVERT(DATETIME2, ''2022-07-29 18:10:57.868877'')) as dbt_valid_to
    from (
        
	
	SELECT * from stg.[Auct_AcceptionApplicationsBits_Inter]
    ) sbq
