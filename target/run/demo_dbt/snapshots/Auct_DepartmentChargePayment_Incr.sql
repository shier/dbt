  ...
  if object_id ('"stg"."Auct_DepartmentChargePayment_Incr"','U') is not null
   ...
   EXEC('create view stg.Auct_DepartmentChargePayment_Incr_temp_view as
    ...
    CONVERT(VARCHAR(32), HashBytes(''MD5'', 
        coalesce(cast(DepartmentChargePaymentID as varchar(max)), '''')  + ''|'' + 
    
        coalesce(cast(CONVERT(DATETIME2, ''2022-07-29 18:17:53.833633'') as varchar(max)), '''') 
    ), 2)
 as dbt_scd_id,
        CONVERT(DATETIME2, ''2022-07-29 18:17:53.833633'') as dbt_updated_at,
        CONVERT(DATETIME2, ''2022-07-29 18:17:53.833633'') as dbt_valid_from,
        nullif(CONVERT(DATETIME2, ''2022-07-29 18:17:53.833633''), CONVERT(DATETIME2, ''2022-07-29 18:17:53.833633'')) as dbt_valid_to
    from (
        
	
	SELECT * from stg.[Auct_DepartmentChargePayment_Inter]
    ) sbq
