  ...
  if object_id ('"stg"."Auct_ConsignmentAssignment_Incr"','U') is not null
   ...
   EXEC('create view stg.Auct_ConsignmentAssignment_Incr_temp_view as
    ...
    CONVERT(VARCHAR(32), HashBytes(''MD5'', 
        coalesce(cast(AssignID as varchar(max)), '''')  + ''|'' + 
    
        coalesce(cast(CONVERT(DATETIME2, ''2022-07-29 18:15:46.544040'') as varchar(max)), '''') 
    ), 2)
 as dbt_scd_id,
        CONVERT(DATETIME2, ''2022-07-29 18:15:46.544040'') as dbt_updated_at,
        CONVERT(DATETIME2, ''2022-07-29 18:15:46.544040'') as dbt_valid_from,
        nullif(CONVERT(DATETIME2, ''2022-07-29 18:15:46.544040''), CONVERT(DATETIME2, ''2022-07-29 18:15:46.544040'')) as dbt_valid_to
    from (
        
	
	SELECT * from stg.[Auct_ConsignmentAssignment_Inter]
    ) sbq
