  ...
  if object_id ('"stg"."CC_SecURItySystemoBJectPermissionsobject_Incr"','U') is not null
   ...
   EXEC('create view stg.CC_SecURItySystemoBJectPermissionsobject_Incr_temp_view as
    ...
    CONVERT(VARCHAR(32), HashBytes(''MD5'', 
        ...
        coalesce(cast(CONVERT(DATETIME2, ''2022-07-29 18:43:03.776502'') as varchar(max)), '''') 
    ), 2)
 as dbt_scd_id,
        CONVERT(DATETIME2, ''2022-07-29 18:43:03.776502'') as dbt_updated_at,
        CONVERT(DATETIME2, ''2022-07-29 18:43:03.776502'') as dbt_valid_from,
        nullif(CONVERT(DATETIME2, ''2022-07-29 18:43:03.776502''), CONVERT(DATETIME2, ''2022-07-29 18:43:03.776502'')) as dbt_valid_to
    from (
        
	
	SELECT * from stg.[CC_SecURItySystemoBJectPermissionsobject_Inter]
    ) sbq
