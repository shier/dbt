  ...
  if object_id ('"dbo"."AH_Invoice__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbo"."AH_Invoice__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbo"."AH_Invoice__dbt_tmp"','U') is not null
    begin
    drop table "dbo"."AH_Invoice__dbt_tmp"
    end


   EXEC('create view dbo.AH_Invoice__dbt_tmp_temp_view as
...
JOIN [stg].[AH_LineItems_FinalView] AS H_LI ON H_LI.[InvoiceID]=H_I.[ID]
  ...
  CREATE TABLE "dbo"."AH_Invoice__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM dbo.AH_Invoice__dbt_tmp_temp_view)
