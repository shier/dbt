...
With Auct_CustomerAccount_Ex AS (
	 select * from stg.[Auct_CustomerAccount_FinalView]
),
Auct_Customer_HistoricalCompanyID_CTE AS (
    SELECT 
        [CustomerAccountID],
    ...
    FROM (
        SELECT 
            ...
            cxacct.[CompanyID]
        FROM [Auct_CustomerAccount_Ex] cxacct -- (Update) need replace [dbo].[Auct_CustomerAccount_Ex] with [dbo].[Auct_CustomerAccount]
